...
def test_basic_add_get_method():
    ...
    init()
    Audio.add_audio_file("1", "name1", "/some/path", 56)
    res = Audio.get_audio_file_info_with_name("name1")
...
def test_basic_add_get_all_method():
    ...
    init()
    Audio.add_audio_file("1", "name1", "/some/path", 56)
    Audio.add_audio_file("2", "name2", "/some/path/2", 17)
    res = Audio.get_all_audio_file_ids()
    assert res == ["1", "2"]
...
def test_basic_add_get_all_with_maxduration_method():
    ...
    Audio.add_audio_file("1", "name1", "/some/path", 56)
    Audio.add_audio_file("2", "name2", "/some/path/2", 17)
    res = Audio.get_all_audio_file_names_and_durations(20)
    assert res == [{'name': 'name2', 'duration': 17}]
    res = Audio.get_all_audio_file_names_and_durations(100)
    assert res == [{'name': 'name1', 'duration': 56}, {'name': 'name2', 'duration': 17}]
    res = Audio.get_all_audio_file_names_and_durations()
    assert res == [{'name': 'name1', 'duration': 56}, {'name': 'name2', 'duration': 17}]
...
def test_basic_add_update_method():
    ...
    init()
    Audio.add_audio_file("1", "name1", "/some/path", 56)
    Audio.update_audio_file("1", "/some/other/path", 17)
    res = Audio.get_audio_file_info_with_name("name1")
    assert res["file_id"] == "1"
